...
var Sentinel2 = ee.Image('COPERNICUS/S2_SR/20190812T135121_20190812T135127_T21KYT');
var image = Sentinel2.select('B.*').clip(roi);

print('1 - Sentinel 2 Image', image);

// Creating visualizations for the Sentinel 2 image
Map.addLayer(image, { min: 134, max: 2870, bands: ['B4', 'B3', 'B2'] }, 'RGB 543', 0);
Map.addLayer(image, { min: 212, max: 3257, bands: ['B8', 'B4', 'B3'] }, 'RGB 753', 0);

// Calculating auxiliary data

// NDVI Index
var ndvi = image.normalizedDifference(['B8', 'B4']).rename('NDVI');
Map.addLayer(ndvi, { min: 0.0, max: 0.85, palette: ["red", "orange", "yellow", "green"] }, "NDVI", 0);
...
Map.addLayer(ndwi, { min: -0.75, max: 0.0, palette: ["white", "gray", "cyan", "blue"] }, "NDWI", 0);

// Gray Level Co-occurrence Matrix - Texture Metrics
var glcm = image.select('B8').glcmTexture({ size: 1, average: true });
...
var correlation = glcm.select('B8_corr');
...
var homogeneity = glcm.select('B8_idm');
...
var entropy = glcm.select('B8_ent');
...
var imcorr1 = glcm.select('B8_imcorr1');
Map.addLayer(imcorr1, { min: -1, max: 1, palette: ['0000FF', 'FF0000'] }, 'IMCORR1', 0);

var imcorr2 = glcm.select('B8_imcorr2');
...
var maxcorr = glcm.select('B8_maxcorr');
...
var inertia = glcm.select('B8_inertia');
...
var shade = glcm.select('B8_shade');
