...
var image = imageModule.mosaic2.select('b3', 'b5', 'b1', 'b2', 'pc1', 'b4', 'NDVI', 'pc5', 'pc2', 'b6', 'pc4', 'pc3', 'NDWI', 'b7_savg', 'b7', 'b8');
...
// Splitting training and testing samples, holdout validation.
var trainingSamples = samples.filter(ee.Filter.lt('random', 0.7));
var testingSamples = samples.filter(ee.Filter.gte('random', 0.7));

Map.addLayer(trainingSamples, {}, 'Training Samples', 0);
Map.addLayer(testingSamples, {}, 'Testing Samples', 0);
...
var chart = ui.Chart.array.values({
  ...
  title: 'Band Importance - Pixel-based approach (WorldView 2)',
  titleTextStyle: {
    bold: true,
    fontSize: 16,
    italic: false
...
Export.image.toDrive({
  ...
  description: 'WorldView2_Class_Pixel_CG',
