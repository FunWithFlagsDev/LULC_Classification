var finalImageModule = require('users/Jhonnattan_Oliveira/Mestrado:modulos_WorldViewe/1 - calculo_indices_metricas');

var roi = ee.FeatureCollection('users/Jhonnattan_Oliveira/CG-MS/LIMITE_CG');

var image = finalImageModule.mosaico1;
...
// Extensive PCA calculation might expire when displayed on the map
// Export the results and import them back
Export.image.toAsset({
  image: pca,
  description: 'Principal_Components_Image',
  assetId: 'users/Jhonnattan_Oliveira/CG-MS/CG_norm_5pca0_5',
  region: geometry,
  scale: scale,
  maxPixels: 1e10});

// Once the export is done, import the asset
var pcaImported = ee.Image('users/Jhonnattan_Oliveira/CG-MS/CG_norm_5pca0_5');
var pcaVisParams = {bands: ['pc1', 'pc2', 'pc3'], min: -2, max: 2};
Map.addLayer(pcaImported, pcaVisParams, 'Principal Components', 0);

// Preparing the final image, adding the Principal Components Analysis (PCA)
